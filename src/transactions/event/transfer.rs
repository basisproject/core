...
pub fn transfer<T: Into<NumericUnion>>(
    caller: &User,
    member: &Member,
    ...
    if !agreement.has_participant(&company_from.agent_id())
        || !agreement.has_participant(&company_from.agent_id())
    {
        // can't create an event for an agreement you are not party to
        Err(Error::InsufficientPrivileges)?;
    }
    let measure = {
        let unit = resource_from
            .get_unit()
    ...
    let mut statebuilder = EventProcessState::builder().resource(resource_from);
    ...
    let state = statebuilder.build().map_err(|e| Error::BuilderFailed(e))?;
...
pub fn transfer_all_rights<T: Into<NumericUnion>>(
    caller: &User,
    member: &Member,
    company_from: &Company,
    company_to: &Company,
    ...
    member.access_check(
        caller.id(),
        company_from.id(),
        CompanyPermission::TransferAllRights,
    )?;
    if !company_from.is_active() {
        Err(Error::ObjectIsInactive("company".into()))?;
    }
...
/// another, moving a set of costs with it.
pub fn transfer_custody<T: Into<NumericUnion>>(
    caller: &User,
    member: &Member,
    company_from: &Company,
    company_to: &Company,
    ...
    member.access_check(
        caller.id(),
        company_from.id(),
        CompanyPermission::TransferCustody,
    )?;
    if !company_from.is_active() {
        Err(Error::ObjectIsInactive("company".into()))?;
    }
...
mod tests {
    ...
    fn can_transfer() {
        ...
        let resource_from = make_resource(
            &ResourceID::new("plank"),
            company_from.id(),
            &Measure::new(dec!(15), Unit::One),
            &Costs::new_with_labor("homemaker", 157),
            &now,
        );
        let resource_to = make_resource(
            &ResourceID::new("plank"),
            company_to.id(),
            &Measure::new(dec!(3), Unit::One),
            &Costs::new_with_labor("homemaker", 2),
            &now,
        );
        state.model = Some(resource_from);
        state.model2 = Some(resource_to);

        let testfn_inner = |state: &TestState<Resource, Resource>,
                            company_from: &Company,
                            company_to: &Company,
                            agreement: &Agreement,
                            resource_to: ResourceMover| {
            transfer(
                ...
                8,
                Some(agreed_in.clone()),
                Some("giving jinkey some post-capitalist planks".into()),
                &now,
            )
        };
        let testfn_update = |state: &TestState<Resource, Resource>| {
            testfn_inner(
                ...
                &company_to,
                &agreement,
                ResourceMover::Update(state.model2().clone()),
            )
        };
        let testfn_create = |state: &TestState<Resource, Resource>| {
            testfn_inner(
                ...
                &company_to,
                &agreement,
                ResourceMover::Create(state.model2().id().clone()),
            )
        };
        ...
        assert_eq!(
            ...
            &Some(company_from.agent_id())
        );
        assert_eq!(
            resource2.inner().accounting_quantity(),
            &Some(Measure::new(dec!(15) - dec!(8), Unit::One))
        );
        assert_eq!(
        ...
        assert_eq!(
            resource_to2.inner().primary_accountable(),
            &Some(company_to.agent_id())
        );
        assert_eq!(
            resource_to2.inner().accounting_quantity(),
            &Some(Measure::new(dec!(8) + dec!(3), Unit::One))
        );
        assert_eq!(
            resource_to2.inner().onhand_quantity(),
            &Some(Measure::new(dec!(8) + dec!(3), Unit::One))
        );
        ...
        assert_eq!(
            resource3.inner().primary_accountable(),
            &Some(company_from.agent_id())
        );
        assert_eq!(
            resource3.inner().accounting_quantity(),
            &Some(Measure::new(dec!(15) - dec!(8), Unit::One))
        );
        assert_eq!(
            resource3.inner().onhand_quantity(),
            &Some(Measure::new(dec!(15) - dec!(8), Unit::One))
        );
        ...
        assert_eq!(
            resource_created.inner().primary_accountable(),
            &Some(company_to.agent_id())
        );
        assert_eq!(
            resource_created.inner().accounting_quantity(),
            &Some(Measure::new(dec!(8), Unit::One))
        );
        assert_eq!(
            resource_created.inner().onhand_quantity(),
            &Some(Measure::new(dec!(8), Unit::One))
        );
        ...
        state2
            .model2_mut()
            .inner_mut()
            .set_primary_accountable(Some(CompanyID::new("zing").into()));
    ...
    fn can_transfer_all_rights() {
        ...
        assert_eq!(
            resource_created.inner().onhand_quantity(),
            &Some(Measure::new(dec!(0), Unit::One))
        );
    ...
    fn can_transfer_custody() {
        ...
        let agreed_in: Url = "https://legaldoom.com/trade-secrets-trade-secrets"
            .parse()
            .unwrap();
        let resource_from = make_resource(
            &ResourceID::new("plank"),
