...
pub fn create<T: Into<String>>(caller: &User, member: &CompanyMember, company: &Company, id: ResourceSpecID, name: T, classifications: Vec<Url>, active: bool, now: &DateTime<Utc>) -> Result<Modifications> {
...
pub fn update(caller: &User, member: &CompanyMember, company: &Company, mut subject: ResourceSpec, name: Option<String>, classifications: Option<Vec<Url>>, active: Option<bool>, now: &DateTime<Utc>) -> Result<Modifications> {
    caller.access_check(Permission::CompanyUpdateResourceSpecs)?;
...
mod tests {
    ...
    fn can_create() {
        ...
        let mods = create(&user, &member, &company, id.clone(), "Beans", vec!["https://www.wikidata.org/wiki/Q379813".parse().unwrap()], true, &now).unwrap().into_modifications();
        ...
        let res = create(&user, &member2, &company, id.clone(), "Beans", vec!["https://www.wikidata.org/wiki/Q379813".parse().unwrap()], true, &now);
        assert_eq!(res, Err(Error::InsufficientPrivileges));

        let mut user2 = user.clone();
        user2.set_roles(vec![]);
        let res = create(&user2, &member, &company, id.clone(), "Beans", vec!["https://www.wikidata.org/wiki/Q379813".parse().unwrap()], true, &now);
        assert_eq!(res, Err(Error::InsufficientPrivileges));

        let mut company2 = company.clone();
        company2.set_deleted(Some(now.clone()));
        let res = create(&user, &member, &company2, id.clone(), "Beans", vec!["https://www.wikidata.org/wiki/Q379813".parse().unwrap()], true, &now);
